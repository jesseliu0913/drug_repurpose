...
def extract_yes_no(answer, prompt_type):
    if "$YES$" in answer:
        return "YES"
    elif "$NO$" in answer:
        return "NO"
...
def calculate_metrics(jsonl_file, ground_truth_file, prompt_type):
    ...
    with jsonlines.open(jsonl_file, 'r') as reader:
        for obj in reader:
            ...
            label = obj.get('label')
            if prompt_type not in ["fcot", "raw"]:
                answer = answer.split("\n")[0]
            all_answers.append(answer)
            prediction_text = extract_yes_no(answer, prompt_type)
...
# python cal_qwq.py  --input_dir ./results --ground_truth ./data_analysis/test_data_new.csv
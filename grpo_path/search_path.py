...
def find_disease_protein_drug_paths(established_pairs):
    ...
    for (disease, drug), relation in established_pairs.items():
        for protein in disease_protein_map.get(disease, set()):
            if drug in protein_drug_map.get(protein, set()):
...
def find_disease_disease_drug_paths(established_pairs):
    ...
    for (disease1, drug), relation in established_pairs.items():
        for disease2 in disease_disease_map.get(disease1, set()):
            if (disease2, drug) in established_pairs:
                disease_disease_paths.append({
                    ...
                    'drug_name': node_name_map[drug],
                    'original_relation': relation,
                    'related_disease_relation': established_pairs[(disease2, drug)],
    ...
    for (disease, drug), relation in established_pairs.items():
        for exposure in disease_exposure_map.get(disease, set()):
            for gene in exposure_gene_map.get(exposure, set()):
                if drug in gene_drug_map.get(gene, set()):
...
pd.DataFrame(disease_protein_drug_paths).to_csv('./path_data/disease_protein_drug_paths.csv', index=False)
pd.DataFrame(disease_phenotype_paths).to_csv('./path_data/disease_phenotype_protein_drug_paths.csv', index=False)
pd.DataFrame(disease_disease_paths).to_csv('./path_data/disease_disease_drug_paths.csv', index=False)
pd.DataFrame(disease_exposure_gene_paths).to_csv('./path_data/disease_exposure_gene_drug_paths.csv', index=False)
pd.DataFrame(disease_exposure_bioprocess_paths).to_csv('./path_data/disease_exposure_bioprocess_gene_drug_paths.csv', index=False)
pd.DataFrame(disease_exposure_molfunc_paths).to_csv('./path_data/disease_exposure_molfunc_gene_drug_paths.csv', index=False)
pd.DataFrame(disease_exposure_cellcomp_paths).to_csv('./path_data/disease_exposure_cellcomp_gene_drug_paths.csv', index=False)

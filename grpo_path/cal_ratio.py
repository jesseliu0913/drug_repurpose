...
established_pairs_df = pd.read_csv('./path_data/established_disease_drug_pairs.csv')

established_dict = {}
for _, row in established_pairs_df.iterrows():
    pair_tuple = ast.literal_eval(row['disease_drug_pair'])
    disease_index, drug_index = pair_tuple
    established_dict[(disease_index, drug_index)] = row['relation']
...
for path_type, file_path in path_files.items():
    try:
        ...
        if path_type == 'disease':
            # Check if disease1_index and drug_index exist in the DataFrame
            if 'disease1_index' in paths_df.columns and 'drug_index' in paths_df.columns:
                for _, row in paths_df.iterrows():
                    pair = (row['disease1_index'], row['drug_index'])
                    pairs_path_types[pair][path_type] += 1
                    pairs_with_paths.add(pair)
                    
                    # Check if original_relation exists
                    if 'original_relation' in paths_df.columns:
                        relation = row['original_relation']
                    else:
                        relation = established_dict.get(pair, "unknown")
                    relation_path_types[relation][path_type] += 1
        else:
            # Check if disease_index and drug_index exist in the DataFrame
            if 'disease_index' in paths_df.columns and 'drug_index' in paths_df.columns:
                for _, row in paths_df.iterrows():
        ...
        print(f"Error processing {path_type} file: {e}")
...
for pair, path_counts in pairs_path_types.items():
    ...
    for path_type, file_path in path_files.items():
        try:
            ...
            if path_type == 'disease':
                matching_rows = paths_df[
                    (paths_df['disease1_index'] == disease_index) & 
                    (paths_df['drug_index'] == drug_index)
                ]
                if not matching_rows.empty:
                    disease_name = matching_rows['disease1_name'].values[0]
                    drug_name = matching_rows['drug_name'].values[0]
                    break
            else:
                matching_rows = paths_df[
                    (paths_df['disease_index'] == disease_index) & 
                    (paths_df['drug_index'] == drug_index)
                ]
                if not matching_rows.empty:
                    disease_name = matching_rows['disease_name'].values[0]
                    drug_name = matching_rows['drug_name'].values[0]
                    break
        except:
            continue
